...
void TFT_Config_Pins(TFT_HandleTypeDef *htft,
					 GPIO_TypeDef dc_port, uint32_t dc_pin,
					 GPIO_TypeDef res_port, uint32_t res_pin,
					 GPIO_TypeDef bl_port, uint32_t bl_pin)
{
...
void TFT_Pin_RES_Set(TFT_HandleTypeDef *htft, uint8_t level)
{
	...
	{ // GPIOA
		if (level)
			GPIOA_SetBits(htft->res_pin);
		else
			GPIOA_ResetBits(htft->res_pin);
	}
	else
	{ // GPIOB
		if (level)
			GPIOB_SetBits(htft->res_pin);
		else
			GPIOB_ResetBits(htft->res_pin);
	}
...
void TFT_Pin_DC_Set(TFT_HandleTypeDef *htft, uint8_t level)
{
	...
	{ // GPIOA
		if (level)
			GPIOA_SetBits(htft->dc_pin);
		else
			GPIOA_ResetBits(htft->dc_pin);
	}
	else
	{ // GPIOB
		if (level)
			GPIOB_SetBits(htft->dc_pin);
		else
			GPIOB_ResetBits(htft->dc_pin);
	}
#elif defined(SOME_OTHER_PLATFORM)
	// 在此添加其他平台的 GPIO 控制代码
...
void TFT_Pin_CS_Set(TFT_HandleTypeDef *htft, uint8_t level)
{
	...
	{ // GPIOA
		if (level)
			GPIOA_SetBits(htft->cs_pin);
		else
			GPIOA_ResetBits(htft->cs_pin);
	}
	else
	{ // GPIOB
		if (level)
			GPIOB_SetBits(htft->cs_pin);
		else
			GPIOB_ResetBits(htft->cs_pin);
	}
#elif defined(SOME_OTHER_PLATFORM)
	// 在此添加其他平台的 GPIO 控制代码
...
void TFT_Pin_BLK_Set(TFT_HandleTypeDef *htft, uint8_t level)
{
	...
	{ // GPIOA
		if (level)
			GPIOA_SetBits(htft->bl_pin);
		else
			GPIOA_ResetBits(htft->bl_pin);
	}
	else
	{ // GPIOB
		if (level)
			GPIOB_SetBits(htft->bl_pin);
		else
			GPIOB_ResetBits(htft->bl_pin);
	}
#elif defined(SOME_OTHER_PLATFORM)
	// 在此添加其他平台的 GPIO 控制代码
...
void TFT_SPI_Send(TFT_HandleTypeDef *htft, uint8_t *data_buffer, uint16_t length, uint8_t wait_completion)
{
	...
	{
		// 使用平台抽象的阻塞式发送函数
		TFT_Platform_SPI_Transmit_Blocking(htft->spi_handle, data_buffer, length, 100); // 使用最大超时时间
